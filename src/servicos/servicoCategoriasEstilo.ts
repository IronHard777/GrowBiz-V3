...
// Carrega todas as miniaturas pré-geradas (scripts/gerar-thumbnails-categorias.mjs) de uma vez.
const modulosThumbnail = import.meta.glob('../assets/categorias/*.png', { eager: true, import: 'default' }) as Record<string, string>;

function resolverThumbnail(slug: string): string {
  const chave = Object.keys(modulosThumbnail).find(caminho => caminho.endsWith(`/${slug}.png`));
  return chave ? modulosThumbnail[chave] : '';
}
